...
const DEFAULT\_TASKS = \[
  { taskname: 'Water', tasktype: 'periodic', notify\_type: 'interval' },
  { taskname: 'Exercise', tasktype: 'daily', notify\_type: 'fixed\_time' },
  { taskname: 'Study', tasktype: 'manual', notify\_type: 'manual' },
  { taskname: 'Midnight Report', tasktype: 'automated', notify\_type: 'fixed\_time' },
  { taskname: 'Quote of the Day', tasktype: 'automated', notify\_type: 'fixed\_time' }
];
...
**Add to .env**: `FRONTEND\_URL=https://yourdomain.com` (for production)
...
app.set('trust proxy', process.env.TRUST\_PROXY || 1);
```
...
```javascript
const VALID\_TIMEZONES = \[
  'Asia/Kathmandu', 'America/New\_York', 'Europe/London', 'Asia/Tokyo', ... // all IANA timezones
];

exports.isValidTimezone = (tz) => VALID\_TIMEZONES.includes(tz);
```
...
* "Copy Command" button
* Link to bot: https://t.me/your\_bot
...
* Create `achievements` table: `(id, userid, badge\_name, unlocked\_at, streak\_length)`
...
* Create `dailychallenge` table: `(id, challengedate, taskid, target\_count, description)`
...
* Returns: `{taskid, current\_streak, longest\_streak, this\_week\_count, this\_month\_count, last\_activity\_time}`
...
* Query: `SELECT userid, SUM(streak) as total\_streak FROM ... ORDER BY total\_streak DESC`
...
* Add to `taskuser`: `reminder\_style` (enum: serious | friendly | sassy | creative | gamer)
...
* Create `coinhistory` table: `(id, userid, coins\_earned, reason, created\_at)`
...
* Add to `taskuser`: `daily\_target` (int), `weekly\_target` (int)
...
* `GET /api/share/badge/:userid/:badge\_name` returns PNG
...
* Create `buddies` table: `(id, user1\_id, user2\_id, created\_at)`
* Endpoint: `POST /api/buddy/invite`, `GET /api/buddy/requests`
* Track shared completion: `GET /api/buddy/stats/:userid/:buddy\_id`
...
* Create `community` \& `community\_members` tables
...
* Collect response latency: (reminder\_sent\_time - task\_logged\_time)
...
-- User Stats (cached)
CREATE TABLE user\_stats\_cache (
  userid UUID PRIMARY KEY,
  current\_streaks JSONB,
  this\_week\_counts JSONB,
...
CREATE TABLE buddies (
  id SERIAL PRIMARY KEY,
  user1\_id UUID REFERENCES userinfo(userid),
  user2\_id UUID REFERENCES userinfo(userid),
...
CREATE TABLE community\_members (
  id SERIAL PRIMARY KEY,
  community\_id INT REFERENCES communities(id),
  userid UUID REFERENCES userinfo(userid),
